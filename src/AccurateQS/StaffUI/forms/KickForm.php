    ...
    public function sendTo(Player $player): void {
        $form = new CustomForm(function(Player $player, ?array $data) {
            ...
            if ($target instanceof Player) {
                $target->kick("§c[StaffUI] §fVous avez été expulsé du serveur.\n§7Raison: §f$reason");
                $player->sendMessage("§a[StaffUI] §fLe joueur §e$targetPlayer §fa été expulsé.");
                $this->plugin->getHistoryManager()->addEntry($targetPlayer, 'Kick', $reason, $player->getName());
            } else {
                $player->sendMessage("§c[StaffUI] §fLe joueur §e$targetPlayer §fn'est pas en ligne.");
            }
        });

        $form->setTitle("§cExpulser un joueur");
        $form->addDropdown("§7Sélectionnez un joueur", $this->getPlayerList());
        $form->addInput("§7Nom du joueur (si autre)", "Entrez le nom du joueur");
        $form->addInput("§7Raison", "Entrez la raison de l'expulsion");
